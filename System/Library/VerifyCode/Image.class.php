...
// +----------------------------------------------------------------------
namespace System\Library\VerifyCode;
/**
 * 图像操作类库
 * @category   ORG
 * @package  ORG
...
class Image {
    ...
    function showImg($imgFile, $text='', $x='10', $y='10', $alpha='50') {
        ...
        if ($info !== false) {
            $createFun = (string)str_replace('/', 'createfrom', $info['mime']);
            $im = $createFun($imgFile);
            if ($im) {
                $ImageFun = (string)str_replace('/', '', $info['mime']);
                //水印开始
                if (!empty($text)) {
                    ...
                    if (is_file($text) && file_exists($text)) {//判断$text是否是图片路径
                        ...
                        $createFun2 = (string)str_replace('/', 'createfrom', $textInfo['mime']);
     ...
     * @param string $session  session操作对象
     * @param string $strtoupper  区分大小写 true or false
     * @return string
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22,$session,$strtoupper) {
        $randval = String::randString($length, $mode);
        if($strtoupper){
            $session->setSession(md5(strtoupper($randval)));
        }else{
            $session->setSession(md5($randval));
        }
        ...
        for ($i = 0; $i < $length; $i++) {
            imagestring($im, 5, $i * $width/5 + 5, mt_rand(1, 8), $randval{$i}, $stringColor);
        }
        return Image::output($im, $type);
    }
    ...
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='simhei.ttf',$session) {
        ...
        if (!is_file($fontface)) {
            $fontface = dirname(__FILE__) . "/font/" . $fontface;
        }
        for ($i = 0; $i < $length; $i++) {
